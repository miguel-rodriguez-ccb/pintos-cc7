...
static struct list waiting_list; // Lista de los threads que estan en espera
...
thread_init (void)
{
  ...
  list_init (&waiting_list); // Inicializamos lista de espera
...
void insert_in_waiting_list(int64_t ticks){
  ...
  thread_actual->time_thread_sleeping = timer_ticks() + ticks;
...
void remove_sleeping_thread(int64_t ticks){
	...
	while(iter != list_end(&waiting_list) ){
		...
		if(ticks >= thread_lista_espera->time_thread_sleeping){
